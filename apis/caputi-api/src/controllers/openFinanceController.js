  ...
  async getDataAccount(req, res) {
    try {
      ...
      if (!institution) {
        return res
          .status(404)
          .json({ error: 'Instituição parceira não encontrada.' });
      }
      ...
      if (!account) {
        return res.status(404).json({
          error: 'Conta não encontrada para este usuário e instituição.',
        });
      }
      ...
      if (!account.consent) {
        return res.status(403).json({
          error:
            'Consentimento para compartilhamento de dados não foi fornecido para esta conta.',
        });
      }
  ...
  async updateConsent(req, res) {
    try {
      ...
      if (typeof consent !== 'boolean') {
        return res
          .status(400)
          .json({ error: 'O valor de "consent" deve ser true ou false.' });
      }
